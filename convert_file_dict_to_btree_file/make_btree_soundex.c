...
{//phân phối lại các từ vào các nhóm-các gốc theo soundex_code
  ...
  if(btree_soundex==NULL) 
    btree_soundex=btcrt("./media/hoang/TONG HOP/C-Advance/PROJECT/Team/Main file_not_use_GTK/Dict_soundex.dat",0,0);

  char word[WORD_LENGTH];
  char mean[1];//yêu cầu phải có mean đưa vào nhưng ko cần dùng đến nên để nó là xâu rỗng.
  ...
  while(btseln(btree, word,mean, 0, &rsize)==0)
    {
      ...
      //printf("%s\t%s\n",soundex_code,word);
      //chỗ này chỉ đưa từ vào thôi, đưa nghĩa vào load rất nặng nên ko đưa vào.
